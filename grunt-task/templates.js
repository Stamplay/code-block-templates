...
module.exports = function templatesFn(grunt) {
  grunt.registerTask('templates', () => {
    ...
    const folders = ['simple', 'context', 'full'];
    folders.map((folderName) => {
      fs.readdirSync(path.join(`templates/${folderName}`)).filter((file) => {
        return file.indexOf('.yaml') >= 0;
      }).map((tmpl) => {
        const template = grunt.file.readYAML(path.join(`templates/${folderName}/`, tmpl));
